...
if ($hasFfplay) {
    ...
    Write-Host '    radiotedu-tui will automatically download a lightweight audio binary on first launch.' -ForegroundColor Gray
    Write-Host '    Alternatively, install ffmpeg: winget install Gyan.FFmpeg' -ForegroundColor Cyan
}
...
if (Get-Command radiotedu -ErrorAction SilentlyContinue) {
    ...
    Write-Host ''
    Write-Host 'To sign in with your RadioTEDU account or device code:' -ForegroundColor White
    Write-Host '  radiotedu login' -ForegroundColor Cyan
    Write-Host '  (Visit https://radiotedu.com/device or https://radiotedu.com/erp/device)' -ForegroundColor Gray
    Write-Host ''
} else {
